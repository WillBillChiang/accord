...
Confidential VM. Implements provable deletion from Conditional Recall:
...
The entire Confidential VM's memory is encrypted by AMD SEV-SNP.
On VM termination, the per-VM encryption key is destroyed, providing
hardware-level provable deletion.
"""
...
from engine.crypto.session_keys import SessionKeyManager
from engine.crypto.secure_delete import secure_zero_dict, secure_zero_list
...
class NegotiationSession:
    """
    Represents a single negotiation session inside the Confidential VM.
    All data exists only in encrypted VM memory and is destroyed on termination.
    """
    session_id: str
    ...
    # Party configurations (decrypted inside Confidential VM only)
    ...
    def get_redacted_log(self) -> list[dict]:
        ...
        Safe to send outside the Confidential VM for audit purposes.
        """
        redacted = []
